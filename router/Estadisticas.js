...
router.get('/mesa-computo/:eventoCatId', checkPermiso('computo', 'ver'), estadisticasController.verMesaComputo);

// 5. OFICIALIZAR CATEGORÍA Y GUARDAR RESULTADOS FINALES
router.post('/oficializar-categoria', checkPermiso('computo', 'editar'), estadisticasController.oficializarCategoria);

// 7. MESA DE CÓMPUTO ESPECÍFICA PARA ABSOLUTOS
router.get('/mesa-computo-absoluto', checkPermiso('computo', 'ver'), estadisticasController.verMesaComputoAbsoluto);

// 8. OFICIALIZAR GANADOR ABSOLUTO (Inyección de puntos a Team)
router.post('/oficializar-absoluto', checkPermiso('computo', 'editar'), estadisticasController.oficializarAbsoluto);

// 10. PANEL DEL PRESIDENTE DE MESA
router.get('/presidente-mesa/:eventoCatId', checkPermiso('computo', 'ver'), estadisticasController.verPresidenteMesa);

// 13. COMPARACIÓN TOP 5 — Vista del Juez (tablet)
router.get('/comparacion/:eventoCatId', checkPermiso('computo', 'ver'), estadisticasController.verComparacionJuez);

// 14. GUARDAR SELECCIÓN TOP 5 DE UN JUEZ
router.post('/guardar-top5', checkPermiso('computo', 'crear'), estadisticasController.guardarTop5);
...
router.get('/certificado-preview/:idCompetidor', checkPermiso('reportes', 'ver'), estadisticasController.verCertificadoPreview);
...
router.post('/:id/calcular/:catId', checkPermiso('computo', 'editar'), estadisticasController.calcularPosiciones);
