...
**No se tocó** (fuera de alcance, código legacy): `verCalculosEvento`/`calculos.ejs` (superado por Mesa de Cómputo). **Corrección 2026-07-31**: `pesajePage`/`crearCompetidor` (`/inscripcion/pesaje`, vista `eventos/pesaje.ejs`) **sí está enrutado y el usuario lo estaba usando en producción** — la nota anterior de "no enrutado desde ninguna vista activa" era incorrecta. Confirmado que su guardado (`_supabase.from('inscripciones').upsert(...)`) escribe en la tabla `inscripciones` — DISTINTA de `competidores` (la que usa todo el resto del sistema: Mesa de Cómputo, listados, resultados) — y que ningún otro archivo del proyecto lee de `inscripciones`. En toda su historia esa tabla solo tiene 1 fila, sin `atleta_id` ni `evento_cat_id` (el código nunca los envía). **Cualquier "pesaje" hecho ahí se pierde silenciosamente** — no aparece en ningún listado ni afecta el resultado real del atleta. Decisión del usuario: no arreglar esta pantalla por ahora — usar exclusivamente `/inscripcion?evento=:id` ("Pesaje Asistido") para todo pesaje real, que sí guarda en `competidores` vía `guardarInscripcionAsistida`. Pendiente para después del evento si se decide arreglarla o eliminarla.
