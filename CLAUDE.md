...
- Cada pestaña muestra **un bloque de tabla por cada etapa presente** (Eliminatoria/Semifinal/Final R1/Final R2), reutilizando `votingService.calcularPosicionesFinales` — el mismo cálculo real de la Mesa de Cómputo operativa, sin reimplementar el algoritmo IFBB una 3ra vez. Los votos descartados por la regla de extremos (5+ jueces) se resaltan (tachado/gris) para ver a simple vista qué juez quedó fuera de la media.
